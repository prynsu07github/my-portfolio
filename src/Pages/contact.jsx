...
const Contact = () => {
  ...
  const sendEmail = (e) => {
    ...
    emailjs
      .sendForm('service_5v1gdkb', 'template_fdk0nmg', form.current , {
        publicKey: 'j2X7GbLqQichs3QhH',
      })
      .then(
        () => {
          console.log('SUCCESS!');
  ...
  return (
    ...
    >
      ...
      <form ref={form} onSubmit={sendEmail} className="bg-[#fff] flex flex-col p-5 sm:p-16 shadow-lg shadow-gray-400 rounded-lg w-full sm:w-8/12 mt-16 sm:mt-32 ">
        ...
        />
        <button type="submit" className="bg-purple-600 px-8 py-3 font-bold text-white text-lg sm:text-xl rounded-lg hover:-translate-y-1 transition duration-150 ease-in-out shadow-md shadow-purple-400 mt-5">
          Submit
        </button>
      </form>
      </div>
